...
void BoardWindow::init( Game* game )
{
    ...
    if ( game ) {
        ...
        QObject::connect( &TO_QACTION(mClearMovesAction),&QAction::triggered, mGame->getTank(), &Tank::clearMoves );
    }
}
...
void BoardWindow::showMenu( QPoint* globalPos, int col, int row )
{
    if ( mGame ) {
        ...
        if ( mMenu.isEmpty() ) {
            ...
            mMenu.addAction( QString("shoot& "), mGame->getTank(), SLOT(&Tank::fire), shootSeq );
